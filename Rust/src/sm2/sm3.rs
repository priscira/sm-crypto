...
impl Sm3 {
  ...
  fn sm3_pad(&self, byt_arrs: &[u8]) -> Vec<u8> {
...
impl DigestTrait for Sm3 {
  ...
  fn digest(&self, byt_arrs: &[u8]) -> Vec<u8> {
    let mut sm3_v: [u32; 8] = [
      0x7380166f, 0x4914b2b9, 0x172442d7, 0xda8a0600, 0xa96f30bc, 0x163138aa, 0xe38dee4d, 0xb0fb0e4e
    ];

    let byt_arrs = self.sm3_pad(byt_arrs);
...
/// - `sm3_k`: `&[u8]`，HMAC密钥
/// - `val`: `&[u8]`，待签名消息
pub fn hmac_sm3(sm3_k: &[u8], val: &[u8]) -> Vec<u8> {
  const BLOCK: usize = 64;

  // 密钥填充
  let mut sm3_k = sm3_k.to_vec();
  if sm3_k.len() > BLOCK {
    sm3_k = Sm3::new().digest(&sm3_k);
  }
  if sm3_k.len() < BLOCK {
  ...
  let inner = Sm3::new().digest(&[i_pad_k, val.to_vec()].concat());
  Sm3::new().digest(&[o_pad_k, inner].concat())
}
