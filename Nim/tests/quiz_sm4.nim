...
proc quizSm4TalkEcb() =
  ...
  let
    cnTalksEcb = encrypt(cnTalks, sm4K)
    enTalksEcb = encrypt(enTalks, sm4K)
  ...
  assert cnTalks == decrypt(cnTalksEcb, sm4K)
  assert enTalks == decrypt(enTalksEcb, sm4K)
...
proc quizSm4ArrEcb() =
  ...
  var
    cnArrsEcb = encrypt(cnArrs, sm4KArr)
    enArrsEcb = encrypt(enArrs, sm4KArr)
  ...
  assert cnArrs == decrypt(cnArrsEcb, sm4KArr)
  assert enArrs == decrypt(enArrsEcb, sm4KArr)
...
proc quizSm4TalkCbc() =
  ...
  let
    cnTalksCbc = encrypt(cnTalks, sm4K, PaddingKind.PKCS7, ModeKind.CBC, iv)
    enTalksCbc = encrypt(enTalks, sm4K, PaddingKind.PKCS7, ModeKind.CBC, iv)
  ...
  assert cnTalks == decrypt(cnTalksCbc, sm4K, PaddingKind.PKCS7, ModeKind.CBC, iv)
  assert enTalks == decrypt(enTalksCbc, sm4K, PaddingKind.PKCS7, ModeKind.CBC, iv)
...
proc quizSm4ArrCbc() =
  ...
  var
    cnArrsCbc = encrypt(cnArrs, sm4KArr, PaddingKind.PKCS7, ModeKind.CBC, ivArrs)
    enArrsCbc = encrypt(enArrs, sm4KArr, PaddingKind.PKCS7, ModeKind.CBC, ivArrs)
  ...
  assert cnArrs == decrypt(cnArrsCbc, sm4KArr, PaddingKind.PKCS7, ModeKind.CBC, ivArrs)
  assert enArrs == decrypt(enArrsCbc, sm4KArr, PaddingKind.PKCS7, ModeKind.CBC, ivArrs)
